...
def checkValidity(str, GG):
    if (str[0] < 0 or str[0] > 7 or str[1] < 0 or str[1] > 13 or str == (0, 0) or str == (1, 0) or str == (
    2, 0) or str == (0, 1)
        or str == (1, 1) or str == (0, 2) or str == (5, 0) or str == (6, 0) or str == (7, 0) or str == (
    6, 1) or str == (7, 1)
        or str == (7, 2) or str == (0, 11) or str == (0, 12) or str == (1, 12) or str == (0, 13) or str == (
    1, 13) or str == (2, 13)
...
def MaxValue(XX, YY, count, ALPHA, BETA, color, ccolor):
    ...
    # print modList
    # print 'about to enter the loop inside of mac value \n'
    ...
    for i in range(len(XX)):
        ...
        for j in range(len(PML)):
            ...
            # print TT
            # print 'about to enter into min value function \n'
...
def ComputerTurn():
    ...
    print 'some prints what are x and y...'
    print x
    print y
    # print NodeGen
    # print PrunInMax
    ...
    print 'baout to enter into the loop that calls max value'
    ...
    if (len(x) == len(T[1])):
        for i in range(len(x)):
            if x[i] != T[1][i]:
                SqID_plus_pieceId = cv.find_overlapping(x[i][0] * SqSize + SqSize / 2, x[i][1] * SqSize + SqSize / 2,
                                                        x[i][0] * SqSize + SqSize / 2, x[i][1] * SqSize + SqSize / 2)
    ...
    if (CapPiece):
        ...
        y.remove(CapPiece)
        SqID_plus_pieceId = cv.find_overlapping(CapPiece[0] * SqSize + SqSize / 2, CapPiece[1] * SqSize + SqSize / 2,
                                                CapPiece[0] * SqSize + SqSize / 2, CapPiece[1] * SqSize + SqSize / 2)
        cv.delete(SqID_plus_pieceId[1])
        ...
        if CapMove and PF in PML:
            ...
            if (CapX or CapY):
                ...
                if temp in x:
                    ...
                    x.remove(temp)
                    SqID_plus_pieceId = cv.find_overlapping(CapX * SqSize + SqSize / 2, CapY * SqSize + SqSize / 2,
                                                            CapX * SqSize + SqSize / 2, CapY * SqSize + SqSize / 2)
                    cv.delete(SqID_plus_pieceId[1])
                    cv.delete(PieceID)
...
def make_squares(cv, start, stop, color, SqSize):
    for j in range(0, 14):
        for i in range(start, stop, 1):
            ...
            if j == 0 or j == 13:
                if i <= 2 or i >= 5:
                    cv.create_rectangle(i * SqSize, j * SqSize, (i + 1) * SqSize, (j + 1) * SqSize, fill="black",
                                        tag="Square")
            if j == 1 or j == 12:
                if i <= 1 or i >= 6:
                    cv.create_rectangle(i * SqSize, j * SqSize, (i + 1) * SqSize, (j + 1) * SqSize, fill="black",
