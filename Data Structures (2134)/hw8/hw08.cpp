    ...
    int checkBalance(){
    	...
    	while(true){
			...
			if (paren.token == '\0'){
				...
				if (myChars.size() == 0){
				    ++errors;
    				cerr << "Error: On line " << paren.theLine << " there is a closing parenthesis, but no opening one." << endl;
				}
				else{
	    ...
	    if (myChars.size() > 0){
	    	++errors;
    		cerr << "Error: On line " << paren.theLine << " there is no closing parenthesis for at least one opening parenthesis." << endl;
	    }
	    return errors;
    }
private:
    ...
    void checkMatch( const Symbol & oParen, const Symbol & cParen ){
		if((oParen.token == '(' && cParen.token == ')') || (oParen.token == '[' && cParen.token == ']') || (oParen.token == '{' && cParen.token == '}')){
    		return;
    	}
    	else{
    		++errors;
    		cerr << "Error: On line " << cParen.theLine << " there is no closing " << oParen.token << '.' << endl;	
    	}
    }   
};
