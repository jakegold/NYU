// Jake Goldstein HW02 Question 1 Programming part
...
IntArray::IntArray(int n): size(n), array(new int[n]){}

// add your methods here

// Copy Constructor
IntArray::IntArray(const IntArray& rhs): size(rhs.size), array(new int[rhs.size]){
    // Copy over every item in the array
...
IntArray::IntArray(IntArray&& rhs): size(rhs.size), array(new int[rhs.size]){ 
    // Set rhs size to 0 (dont think that this is necessary)
    rhs.size = 0;
    // Have array point to the rhs array, and the rhs array to nullptr
    array = rhs.array;
    rhs.array = nullptr;
} 
// Destructor
...
IntArray& IntArray::operator=(const IntArray& rhs){
    ...
    if (this != &rhs){
        // Delete the array
        delete [] array;
        // Re-size the array to the rhs size
        size = rhs.size;
        array = new int[size];
        // Copy over every item in the array
        for (int i = 0; i < size; ++i){
