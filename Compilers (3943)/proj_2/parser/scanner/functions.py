...
def scanner(std_input, table):
    ...
    for sub_string in smaller_strings:
        # Scan each of these smaller strings
...
# Scans a small string with no whitespace
# Input: a string (no whitespace)
...
def sub_scan(string, table):
    ...
    while start < len(string):
        ...
        if match[1] is 14:
            temp_sub_string = string[start + 1:end + 1]
            temp_match = find_match(temp_sub_string, table)
            if temp_match[1] is 15:
                end = end + 1
        ...
        new_match = find_match(string[start:end_index], table)
        if new_match is not match:
            match = new_match
...
def output(result):
    ...
    for row in result:
        ...
        if not comment:
            if row[1] is 10 and row[3] is not 'int':
                print row[2], '             ', row[3]
            else:
                print row[2]
