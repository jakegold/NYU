			...
			try:
				for var in rhs:
					...
					if lhs_type != rhs_type:
						if lhs_type is None:
							print 'Trying to assign to variable ' + lhs + ' on line ' + str(line_number) + ' but was declared incorrectly'
						elif rhs_type is None:
							print 'Type error on line ' + str(line_number) + ': trying to assign "' + lhs + '" of type ' + lhs_type + ' (declared on line ' + str(lhs_line_dec) + ' ) with "' + var + '" which was declared incorrectly.'
						else:
